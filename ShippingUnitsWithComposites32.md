...
LEFT JOIN [txn].[references] AS [r1] ON [s].[id] = [r1].[shipping_unit_id]
...
ORDER BY [s].[id], [r].[id], [d].[id], [e].[id], [e0].[id], [g].[id], [s0].[id], [s1].[id], [t].[id], [a].[id], [r0].[id], [d0].[id], [e1].[id], [e2].[id], [g0].[id], [s2].[id], [s3].[id], [t0].[id], [s4].[id], [s5].[id]',N'@__batchId_1 bigint',@__batchId_1=200000
```